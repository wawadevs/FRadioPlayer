...
@objc public protocol FRadioPlayerDelegate: class {
    ...
    @objc optional func radioPlayer(_ player: FRadioPlayer, duration: Double)

    @objc optional func radioPlayer(_ player: FRadioPlayer, nextPodcast: Bool)
}
...
open class FRadioPlayer: NSObject {
    ...
    private func playerItemDidChange() {
        ...
        if let item = playerItem {
            ...
            player?.addPeriodicTimeObserver(forInterval: interval, queue: DispatchQueue.main) { [weak self] time in
                ...
                self.delegate?.radioPlayer?(self, duration: time.seconds)
            }
    ...
    @objc func playerItemDidReachEnd(notification: Notification) {
        if let playerItem = notification.object as? AVPlayerItem {
            self.delegate?.radioPlayer?(self, nextPodcast: true)
        }
    }
    ...
    override open func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if let player = object as? AVPlayer, let keyPath = keyPath {
            switch keyPath {
